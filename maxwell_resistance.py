...
# eta = 1/R where R is resistance (constant)
# Test range: R in [1e-6, 1e8]
# Run with: mpirun -n 4 python3 maxwell_resistance.py
...
# 3) Parameters
#    eta = 1/R where R is resistance (constant).
#    Re(eta) > 0 required. See eqn 1.6 in proposal notes.
#    Test range: R in [1e-6, 1e8]
omega, mu, eps = 10.0, 1.0, 1.0
R = 1e-6            # <-- Resistance: vary from 1e-6 to 1e8
eta = PETSc.ScalarType(1.0 / R)
mu_inv = 1.0 / mu
...
if comm.rank == 0: print(f"R = {R:.2e}, eta = 1/R = {1.0/R:.2e}, Dissipation P: {P:.6e}")
...
try:
    ...
    with XDMFFile(comm, f"E_solution_R{R:.2e}.xdmf", "w") as xdmf:
        xdmf.write_mesh(domain)
        xdmf.write_function(E_vis)

    if comm.rank == 0:
        print(f"Successfully wrote E_solution_R{R:.2e}.xdmf")
